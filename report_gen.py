...
<body class="bg-slate-50 text-slate-800">
    ...
    <div class="max-w-7xl mx-auto p-8 space-y-8">
        ...
        <!-- Pivot Table: Clients vs Months -->
        <div class="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden mt-8 page-break">
            <div class="p-6 border-b border-slate-100">
                <h3 class="text-lg font-bold">Tableau Croisé : Top Clients par Mois</h3>
                <p class="text-xs text-slate-400">Valeurs mensuelles en k (milliers) XAF</p>
            </div>
            <div class="overflow-x-auto">
                <table class="w-full text-xs text-left">
                    <thead class="bg-slate-50 text-slate-600 uppercase" id="pivot-thead">
                        <!-- JS generated -->
                    </thead>
                    <tbody id="pivot-tbody" class="divide-y divide-slate-100">
                        <!-- JS generated -->
                    </tbody>
                </table>
            </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden mt-8 page-break">
            <div class="p-6 border-b border-slate-100">
                <h3 class="text-lg font-bold">Tableau Matriciel : Top Produits par Mois</h3>
                <p class="text-xs text-slate-400">Valeurs mensuelles en k (milliers) XAF</p>
            </div>
            <div class="overflow-x-auto">
                <table class="w-full text-xs text-left">
                    <thead class="bg-slate-50 text-slate-600 uppercase" id="pivot-product-thead">
                        <!-- JS generated -->
                    </thead>
                    <tbody id="pivot-product-tbody" class="divide-y divide-slate-100">
                        <!-- JS generated -->
                    </tbody>
                </table>
            </div>
    ...
    <script>
        ...
        // Aggregate by Month
        const monthlyData = {{}};
        monthNames.forEach(m => monthlyData[m] = {{ revenue: 0, qty: 0, clients: {{}}, products: {{}} }});
        ...
        rawData.forEach(row => {{
            ...
            if (monthlyData[m]) {{
                ...
                // Track client per month
                if (!monthlyData[m].clients[row.client]) monthlyData[m].clients[row.client] = 0;
                monthlyData[m].clients[row.client] += row.revenue;

                // Track product per month
                if (!monthlyData[m].products[row.product]) monthlyData[m].products[row.product] = 0;
                monthlyData[m].products[row.product] += row.revenue;
            }}
        ...
        const pivotThead = document.getElementById('pivot-thead');
        const pivotTbody = document.getElementById('pivot-tbody');
        
        // Header Row
        let headerHtml = '<tr><th class="px-4 py-3 sticky left-0 bg-slate-50">Client</th>';
        monthNamesFr.forEach(m => headerHtml += `<th class="px-2 py-3 text-right">${{m.substring(0,3)}}</th>`);
        headerHtml += '<th class="px-4 py-3 text-right">Total</th></tr>';
        pivotThead.innerHTML = headerHtml;

        // Rows (Top 10 Clients)
        sortedClients.forEach(([client, total]) => {{
            let rowHtml = `<tr><td class="px-4 py-3 font-medium sticky left-0 bg-white border-r border-slate-100 truncate max-w-xs" title="${{client}}">${{client}}</td>`;
            monthNames.forEach(m => {{
                const val = (monthlyData[m].clients[client] || 0);
                rowHtml += `<td class="px-2 py-3 text-right ${{val > 0 ? 'text-slate-900' : 'text-slate-300'}}">${{val > 0 ? (val/1000).toFixed(0) : '-'}}</td>`;
            }});
            rowHtml += `<td class="px-4 py-3 text-right font-bold">${{formatMoney(total)}}</td></tr>`;
            pivotTbody.innerHTML += rowHtml;
        }});

        // --- Populate Product Pivot Table ---
        const pivotProductThead = document.getElementById('pivot-product-thead');
        const pivotProductTbody = document.getElementById('pivot-product-tbody');
        
        // Reuse header html but replace Client with Product
        pivotProductThead.innerHTML = headerHtml.replace('Client', 'Produit');

        const top10Products = sortedProducts.slice(0, 10);
        top10Products.forEach(([prod, total]) => {{
            let rowHtml = `<tr><td class="px-4 py-3 font-medium sticky left-0 bg-white border-r border-slate-100 truncate max-w-xs" title="${{prod}}">${{prod}}</td>`;
            monthNames.forEach(m => {{
                const val = (monthlyData[m].products[prod] || 0);
                rowHtml += `<td class="px-2 py-3 text-right ${{val > 0 ? 'text-slate-900' : 'text-slate-300'}}">${{val > 0 ? (val/1000).toFixed(0) : '-'}}</td>`;
            }});
            rowHtml += `<td class="px-4 py-3 text-right font-bold">${{formatMoney(total)}}</td></tr>`;
            pivotProductTbody.innerHTML += rowHtml;
        }});
